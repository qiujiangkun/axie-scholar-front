...
const donutChartColor = ["#83B5D1", "#231F20", "#C1DAE8", "#FFC580", "#FF8B00"];
...
const DonutChartWrap = styled.div`
  padding: 32px 29px 23px 32px;
  border: 1px solid #dddfe5;
  border-radius: 12px;
  min-height: 494px; ;
`;

const DonutChartTitle = styled.div`
...
const Home = () => {
  const [listDetail] = useState([
    { icon: TopScholarIcon, number: "245,202", text: "Total SLP Earned" },
    { icon: TotalSLPIcon, number: "147,121", text: "Total Scholar SLP" },
    { icon: TotalPointIcon, number: "98,081", text: "Total Manager SLP" },
  ]);

  const [donutChartData, setDonutChartData] = useState({
    series: [763, 321, 69, 154, 696],
    options: {
      colors: donutChartColor,
      responsive: [
        {
          breakpoint: 480,
          options: {
    ...
    <Wrapper>
      <Row gutter={[25, 8]}>
        ...
        <Col lg={16} md={16} sm={24} xs={24}>
          <DonutChartWrap>
            <DonutChartTitle>Last 100 Battles</DonutChartTitle>
            <Chart
      ...
      <LineChartWrapper>
          ...
          <Col lg={12} sm={24} xs={24}>
            <ChartHeaderRight>
              <ChartLegend>
                <ChartLegendWrapper>
                  <CustomIcon
                    className="text-primary font-size-xl icon-mobile"
                    svg={ScholarLegendIcon}
                  />
        ...
        <Chart
          options={SLPEarnedData.options}
          series={SLPEarnedData.series}
          height={472}
        />
      </LineChartWrapper>
